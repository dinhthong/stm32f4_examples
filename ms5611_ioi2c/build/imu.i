#line 1 "..\\HardWareDriver\\IMU\\IMU.c"
...
#line 1 "..\\HardWareDriver\\IMU\\IMU.h"
#line 26 "..\\HardWareDriver\\IMU\\IMU.h"
...
#line 27 "..\\HardWareDriver\\IMU\\IMU.c"
...
void IMU_getValues(float *values)
{
	...
	for(i = 0; i < 6; i++)
	{
		if(i < 3)
		{
			...
			values[i] = (float) accgyroval[i] * 0.4f + lastacc[i] * 0.6f ;
			lastacc[i] = values[i];
		}
		else
		{
...
void IMU_AHRSupdate(volatile float gx, volatile float gy, volatile float gz, volatile float ax, volatile float ay, volatile float az, volatile float mx, volatile float my, volatile float mz)
{
	...
	{
		exInt = exInt + ex * 0.03f * halfT;
		eyInt = eyInt + ey * 0.03f * halfT;
		ezInt = ezInt + ez * 0.03f * halfT;
...
void FreeIMU_AHRSupdate(volatile float gx, volatile float gy, volatile float gz, volatile float ax, volatile float ay, volatile float az)
{
	...
	if(ex != 0.0f && ey != 0.0f && ez != 0.0f)
	{
		...
		integralFBx +=  ex * (0.2f) * halfT;
		integralFBy +=  ey * (0.2f) * halfT;
		integralFBz +=  ez * (0.2f) * halfT;

		gx = gx + (1.0f ) * ex + integralFBx;
		gy = gy + (1.0f ) * ey + integralFBy;
		gz = gz + (1.0f ) * ez + integralFBz;
...
void IMU_getQ(float *q,volatile float IMU_values[9])
{
	...
	IMU_AHRSupdate(IMU_values[3] * (float)3.1415926535 / 180, IMU_values[4] * (float)3.1415926535 / 180, IMU_values[5] * (float)3.1415926535 / 180,
		       IMU_values[0], IMU_values[1], IMU_values[2], IMU_values[6], IMU_values[7], IMU_values[8]);

	FreeIMU_AHRSupdate(IMU_values[3] * (float)3.1415926535 / 180, IMU_values[4] * (float)3.1415926535 / 180, IMU_values[5] * (float)3.1415926535 / 180,
...
void IMU_getRPY_normal(float *angles)
{
    ...
    angles[0]+= rateroll * elapsedT * 2000/32767;
    angles[1]+= ratepitch * elapsedT * 2000/32767;
    angles[2]+= rateyaw * elapsedT * 2000/32767;
    
		
    angles[1]-= angles[0]*sin(rateyaw * elapsedT * (2000/32767) * (float)3.1415926535/180);
    angles[0]+= angles[1]*sin(rateyaw * elapsedT * (2000/32767) * (float)3.1415926535/180);
    ...
    angles[1] = angles[1] * 0.975f +  kalAngleY * 0.025f;
    angles[0] =  angles[0] * 0.975f +   kalAngleX * 0.025f ;
}
void IMU_getRollPitchYaw(float *angles)
{
	static float q[4];
	...
	IMU_Roll = angles[0] = (atan2(2.0f * (q[0] * q[1] + q[2] * q[3]),
				      1 - 2.0f * (q[1] * q[1] + q[2] * q[2]))) * 180 / (float)3.1415926535;
	
	
	
	IMU_Pitch = angles[1] = -safe_asin(2.0f * (q[0] * q[2] - q[3] * q[1])) * 180 / (float)3.1415926535;
	IMU_Yaw = angles[2] = -atan2(2 * q1 * q2 + 2 * q0 * q3, -2 * q2 * q2 - 2 * q3 * q3 + 1) * 180 / (float)3.1415926535; 
}
...
void Initialize_Q(void)
{
	...
	lastUpdate = ((TIM_TypeDef *) (((uint32_t)0x40000000) + 0x0C00))->CNT;
}
...
float safe_asin(float v)
{
	if (((sizeof(v) == sizeof(float)) ? __ARM_isnanf(v) : __ARM_isnan(v)))
	{
		return 0.0f;
	}
	if (v >= 1.0f)
	{
		return (float)3.1415926535 / 2;
	}
	if (v <= -1.0f)
	{
		return -(float)3.1415926535 / 2;
