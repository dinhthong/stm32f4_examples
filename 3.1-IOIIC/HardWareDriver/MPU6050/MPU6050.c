...
#define  Buf_Size 10
#define SCLCLOCK RCC_AHB1Periph_GPIOC
#define SCLPORT GPIOC
#define SCLPIN 6
#define SDACLOCK RCC_AHB1Periph_GPIOC
#define SDAPORT GPIOC
#define SDAPIN 7
...
IOIIC_Pin_t mpu6050_iic;

void mpu6050_iic_config(void)
{
	// procedure for IOIIC initialization.
	mpu6050_iic.SCL_Port = SCLPORT;
	mpu6050_iic.SDA_Port = SDAPORT;
	
	mpu6050_iic.SCL_Pin = SCLPIN;
	mpu6050_iic.SDA_Pin = SDAPIN;

	GPIO_InitTypeDef GPIO_InitStructure;
	RCC_AHB1PeriphClockCmd(SCLCLOCK, ENABLE);
	GPIO_InitStructure.GPIO_Pin = (1<<SCLPIN);
	...
	RCC_AHB1PeriphClockCmd(SDACLOCK, ENABLE);
	GPIO_InitStructure.GPIO_Pin = (1<<SDAPIN);
...
{
	IICwriteBits(devAddr, MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_CLKSEL_BIT, MPU6050_PWR1_CLKSEL_LENGTH, source, &mpu6050_iic);
}
...
{
	IICwriteBits(devAddr, MPU6050_RA_GYRO_CONFIG, MPU6050_GCONFIG_FS_SEL_BIT, MPU6050_GCONFIG_FS_SEL_LENGTH, range, &mpu6050_iic);
}


void MPU6050_setFullScaleAccelRange(uint8_t range)
{
	IICwriteBits(devAddr, MPU6050_RA_ACCEL_CONFIG, MPU6050_ACONFIG_AFS_SEL_BIT, MPU6050_ACONFIG_AFS_SEL_LENGTH, range, &mpu6050_iic);
}
...
{
	IICwriteBit(devAddr, MPU6050_RA_PWR_MGMT_1, MPU6050_PWR1_SLEEP_BIT, enabled, &mpu6050_iic);
}
...
{
	IICreadBytes(devAddr, MPU6050_RA_WHO_AM_I, 1, buffer, &mpu6050_iic);
	return buffer[0];
}
...
{
	IICwriteBit(devAddr, MPU6050_RA_USER_CTRL, MPU6050_USERCTRL_I2C_MST_EN_BIT, enabled, &mpu6050_iic);
}

void MPU6050_setI2CBypassEnabled(uint8_t enabled)
{
	IICwriteBit(devAddr, MPU6050_RA_INT_PIN_CFG, MPU6050_INTCFG_I2C_BYPASS_EN_BIT, enabled, &mpu6050_iic);
}
...
void MPU6050_initialize(void)
{
	...
	IICwriteBit(devAddr, MPU6050_RA_INT_PIN_CFG, MPU6050_INTCFG_INT_LEVEL_BIT, 0, &mpu6050_iic);
	IICwriteBit(devAddr, MPU6050_RA_INT_PIN_CFG, MPU6050_INTCFG_INT_OPEN_BIT, 0, &mpu6050_iic);
	IICwriteBit(devAddr, MPU6050_RA_INT_PIN_CFG, MPU6050_INTCFG_LATCH_INT_EN_BIT, 1, &mpu6050_iic);
	IICwriteBit(devAddr, MPU6050_RA_INT_PIN_CFG, MPU6050_INTCFG_INT_RD_CLEAR_BIT, 1, &mpu6050_iic);
	// When DATA is READY -> DRY pin <-> PC9 is set
	IICwriteBit(devAddr, MPU6050_RA_INT_ENABLE, MPU6050_INTERRUPT_DATA_RDY_BIT, 1, &mpu6050_iic);
	//增加MPU6050的低通滤波，大大减少了机架震动对姿态解算的影响
	IICwriteBits(devAddr, MPU6050_RA_CONFIG, 7, 8, MPU6050_DLPF, &mpu6050_iic); //设置MPU6050低通滤波
...
//	}
//	else return 0;
	return 1; // for test with stanalone MPU6050 which doesn't have DRY
}
...
// If DATA READY, then get
	if(MPU6050_is_DRY())
	{
		IICreadBytes(devAddr, MPU6050_RA_ACCEL_XOUT_H, 14, buffer, &mpu6050_iic);
